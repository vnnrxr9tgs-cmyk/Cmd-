...
def process_file(path, out_dir):
    try:
        ...
        # Проверка, если уже моно и нужная частота — пропускаем
        if info.channels == 1 and sr == TARGET_SR and info.subtype == TARGET_SUBTYPE:
            logging.info("  Файл уже в нужном формате, пропускаю.")
            return False
        ...
        # Ресэмплинг с точным рациональным коэффициентом (лучше для качества)
        if sr != TARGET_SR:
            # Вычисляем рациональный коэффициент (пример для 8000→9600: 6/5)
            from fractions import Fraction
            ratio = Fraction(TARGET_SR, sr)
            up, down = ratio.numerator, ratio.denominator
            data = resample_poly(data, up, down)
            logging.info(f"  Ресэмплинг: {sr} - {TARGET_SR} Гц (up={up}, down={down})")

        # Нормализация для улучшения SNR (масштабируем к 0.99, чтобы избежать клиппинга)
...
def main():
    ...
    out_dir = "out2"  # Исправил опечатку "ou2t" на "out2"
