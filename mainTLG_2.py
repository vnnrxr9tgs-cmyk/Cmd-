...
Theme = DarkTheme  # текущая тема (меняется динамически)


def apply_theme(theme_cls):
    global Theme
    Theme = theme_cls
    ...
    def load(self):
        ...
        try:
            ...
            if theme_elem is not None and theme_elem.text:
                self.theme = theme_elem.text.strip().lower()
...
class NewsListModel(QAbstractListModel):
    """Модель с поддержкой постраничной подгрузки."""

    def __init__(self):
        super().__init__()
        self._news: List[News] = []          # все новости текущего канала/поиска (отсортированы)
    ...
    def setFullNews(self, news_list: List[News], page_size: int = PAGE_SIZE):
        """Задать полный список и показать первую страницу."""
        self.beginResetModel()
        self._news = news_list
        self._visible_count = min(page_size, len(news_list))
        self.endResetModel()

    def appendPage(self, page_size: int = PAGE_SIZE) -> bool:
        """Подгрузить следующую страницу. Возвращает True, если ещё есть данные."""
        if self._visible_count >= len(self._news):
            return False
        old = self._visible_count
        new = min(old + page_size, len(self._news))
        self.beginInsertRows(QModelIndex(), old, new - 1)
        self._visible_count = new
        self.endInsertRows()
        return self._visible_count < len(self._news)

    def hasMore(self) -> bool:
        return self._visible_count < len(self._news)

    def fullCount(self) -> int:
...
class ChannelDelegate(QStyledItemDelegate):
    def paint(self, painter, option, index):
        ...
        max_lines = max(1, preview_rect.height() // line_height)
...
class NewsDelegate(QStyledItemDelegate):
    ...
    def _cache_key(self, news: News, width: int) -> str:
        return f"{news.checksum()}:{width}:{self.font_size}"
    ...
    def sizeHint(self, option, index):
        ...
        width = max(100, option.rect.width())
        key = self._cache_key(news, width)
        if key in self._height_cache:
            return QSize(width, self._height_cache[key])
        ...
        total_height += 8
        h = max(100, int(total_height))
        self._height_cache[key] = h
        # ограничим кэш
        if len(self._height_cache) > 2000:
...
class Sidebar(QFrame):
    ...
    def __init__(self, storage):
        ...
        self.title.setStyleSheet(f"color: {Theme.BLUE}; letter-spacing: 0.8px;")
...
class MainContent(QWidget):
    ...
    def __init__(self, font_size=14, sound_enabled=True):
        ...
        self.refresh_btn = self._make_icon_btn("⟳")
        self.refresh_btn.clicked.connect(self.refreshRequested)
        header_layout.addWidget(self.refresh_btn)

        self.sound_btn = self._make_icon_btn("")
        ...
        self.theme_btn = self._make_icon_btn("☀" if Theme.name == "dark" else "🌙")
        ...
        # Подгрузка при прокрутке вниз
        self.news_list.verticalScrollBar().valueChanged.connect(self._on_scroll)
    ...
    def _make_icon_btn(self, text: str) -> QPushButton:
        btn = QPushButton(text)
        btn.setFixedSize(36, 36)
        btn.setCursor(Qt.PointingHandCursor)
        btn.setFont(font_s(16))
        self._style_icon_btn(btn)
        return btn

    def _style_icon_btn(self, btn: QPushButton):
        btn.setStyleSheet(f"""
            QPushButton {{
                background: {Theme.CARD};
                border: 1px solid {Theme.BORDER};
            ...
            QPushButton:pressed {{ background: {Theme.BLUE}; color: white; }}
        """)
    ...
    def apply_theme(self):
        ...
        self.count_label.setStyleSheet(f"color: {Theme.SUBTEXT};")
        self.placeholder.setStyleSheet(f"color: {Theme.SUBTEXT}; background: transparent;")
        for btn in (self.refresh_btn, self.sound_btn, self.theme_btn, self.settings_btn):
            self._style_icon_btn(btn)
    ...
    def _on_scroll(self, value):
        ...
        if bar.maximum() <= 0:
            return
        # когда осталось ~15% до конца
        if value >= bar.maximum() * 0.85 and self.news_model.hasMore():
            self._loading_more = True
            self.loadMoreRequested.emit()
            QTimer.singleShot(100, lambda: setattr(self, "_loading_more", False))
    ...
    def paintEvent(self, event):
        ...
        gradient.setColorAt(0, QColor(Theme.BG_GRAD_TOP))
        gradient.setColorAt(1, QColor(Theme.BG_GRAD_BOTTOM))
    ...
    def showNews(self, news_list: List[News], scroll_to_top=True):
        ...
        self.news_delegate.clear_cache()
        if scroll_to_top:
            self.news_model.setFullNews(news_list, PAGE_SIZE)
            QTimer.singleShot(0, lambda: self.news_list.scrollToTop())
        else:
            scrollbar = self.news_list.verticalScrollBar()
            pos = scrollbar.value()
            # при обновлении сохраняем уже видимое количество, но не меньше PAGE_SIZE
            prev_visible = max(PAGE_SIZE, self.news_model.rowCount())
            self.news_model.setFullNews(news_list, prev_visible)
            QTimer.singleShot(0, lambda: scrollbar.setValue(min(pos, scrollbar.maximum())))

    def loadMore(self):
        if self.news_model.appendPage(PAGE_SIZE):
            pass  # UI обновится через insertRows
...
class MainWindow(QMainWindow):
    ...
    def refresh_ui(self, preserve_selection=False):
        ...
        if preserve_selection and self.selected_channel_name:
            ...
            if self._search_text:
                news = self.storage.search(self._search_text, self.selected_channel_name)
    ...
    def onChannelSelected(self, channel: Channel):
        ...
        # лёгкое обновление unread без полного reload
        self.storage.refresh_unread_only()
        self.sidebar.setChannels(self.storage.channels)
        ...
        self.content.showNews(news)  # первая страница
    ...
    def onSearch(self, text: str):
        ...
        news = self.storage.search(self._search_text,
    ...
    def open_settings(self):
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            self.content.set_sound_icon(self.storage.settings.sound_enabled)
            # тема
            if self.storage.settings.theme == "light":
                apply_theme(LightTheme)
            else:
                apply_theme(DarkTheme)
            self.apply_theme()
...
# ---------------------------------------------------------------------------
# Storage — оптимизированная загрузка
# ---------------------------------------------------------------------------
...
class NewsStorage:
    def __init__(self, data_dir: str, settings: Optional[Settings] = None):
        ...
        # кэш: channel -> (mtime, list[News], file_hash)
        self._channel_cache: Dict[str, Tuple[float, List[News], str]] = {}
    ...
    def mark_all_current_as_read(self):
        ...
        for channel in self.get_all_channel_names():
            news_list = self._load_channel_news_cached(channel)
            if news_list:
    ...
    def _file_hash(self, path: str) -> str:
        """Быстрый хэш файла (первые + последние 4KB + размер)."""
        try:
    ...
    def _load_channel_news_cached(self, channel_name: str) -> List[News]:
        path = self.channel_file_path(channel_name)
        if not os.path.exists(path):
            self._channel_cache.pop(channel_name, None)
            return []
        try:
        ...
        # mtime изменился — проверяем content-hash, чтобы отсечь «пустые» перезаписи
        fhash = self._file_hash(path)
        if cached and cached[2] == fhash:
            # тот же контент, просто обновляем mtime
            self._channel_cache[channel_name] = (mtime, cached[1], fhash)
            return cached[1]

        news_list = self._parse_channel_file(path, channel_name)
        self._channel_cache[channel_name] = (mtime, news_list, fhash)
        return news_list

    def _parse_channel_file(self, path: str, channel_name: str) -> List[News]:
        try:
            ...
            for item in data:
                ...
                news_list.append(News(date=date, channel=channel_name, text=text, title=title))
    ...
    def load(self):
        self.news, self.channels, self.last_read_by_channel = self.load_news_data(
            self.data_dir, self.settings, self.last_read_by_channel, self._channel_cache
        )

    def refresh_unread_only(self):
        """Быстро пересчитать unread без перечитывания файлов."""
        for ch in self.channels:
            channel_news = [n for n in self.news if n.channel == ch.name]
            ch.unread_count = self._unread_count_for_channel(
                ch.name, channel_news, self.last_read_by_channel
            )
            ch.notify_enabled = self.settings.is_notify(ch.name)
            ch.sound_enabled = self.settings.is_sound(ch.name)

    @staticmethod
    def load_news_data(data_dir, settings, last_read_by_channel, channel_cache=None):
        ...
        for channel in all_channel_names:
            ...
            # используем кэш storage, если передан
            if channel_cache is not None and hasattr(channel_cache, "get"):
                # channel_cache — это dict storage._channel_cache, но в staticmethod
                # мы работаем через вспомогательную логику ниже
                pass

            news_list = NewsStorage._load_channel_news(data_dir, channel, channel_cache)
    ...
    @staticmethod
    def _load_channel_news(data_dir, channel_name, channel_cache=None):
        safe_name = channel_name.replace("/", "_").replace("\\", "_")
        path = os.path.join(data_dir, f"{safe_name}.json")
        if not os.path.exists(path):
            if channel_cache is not None:
                channel_cache.pop(channel_name, None)
            return []
        ...
        if channel_cache is not None:
            cached = channel_cache.get(channel_name)
            if cached and cached[0] == mtime:
                return cached[1]
            # content hash
            try:
                ...
                with open(path, "rb") as f:
                    ...
                    if size > 8192:
                        ...
                        tail = b""
                fhash = hashlib.md5(head + tail + str(size).encode()).hexdigest()
            except Exception:
                fhash = ""
            if cached and cached[2] == fhash:
                channel_cache[channel_name] = (mtime, cached[1], fhash)
                return cached[1]
        ...
        try:
            ...
            if channel_cache is not None:
                try:
                    fhash = hashlib.md5(open(path, "rb").read(4096) + str(os.path.getsize(path)).encode()).hexdigest()
                except Exception:
                    fhash = ""
                channel_cache[channel_name] = (mtime, news_list, fhash)
            return news_list
        except Exception:
            return []
    ...
    def _unread_count_for_channel(channel_name, channel_news, last_read_by_channel):
        ...
        if not last_read_date_str:
            return 0
        last_read_dt = parse_dt(last_read_date_str)
        if last_read_dt == datetime.min:
            return 0
    ...
    def search(self, text, channel=None):
        ...
        return [n for n in data if text in n.text.lower() or (n.title and text in n.title.lower())]
...
class NewsLoaderThread(QThread):
    ...
    def __init__(self, data_dir, settings, last_read_by_channel, channel_cache):
        super().__init__()
        self.data_dir = data_dir
        self.settings = settings
        self.last_read = last_read_by_channel.copy()
        # передаём ссылку на кэш — поток будет читать/писать аккуратно
        self.channel_cache = channel_cache

    def run(self):
        try:
            # копия кэша для потока, чтобы не блокировать UI надолго
            local_cache = dict(self.channel_cache)
            news, channels, updated_last_read = NewsStorage.load_news_data(
                self.data_dir, self.settings, self.last_read, local_cache
            )
            # возвращаем обновлённый кэш
            self.finished_with_result.emit((news, channels, updated_last_read, local_cache))
        except Exception as e:
...
class SettingsDialog(QDialog):
    def __init__(self, channels, settings, parent=None):
        ...
        data_label.setStyleSheet(f"color: {Theme.TEXT};")
        ...
        notify_label.setStyleSheet(f"color: {Theme.TEXT};")
        ...
        global_layout.addLayout(theme_layout)
    ...
    def browse_folder(self):
        folder = QFileDialog.getExistingDirectory(self, "Выберите папку с новостями",
...
class TrayApp(QApplication):
    def __init__(self, argv):
        ...
        if not self._data_available:
            QMessageBox.warning(
                ...
                f"Вы можете изменить путь в настройках."
            )
        ...
        self.last_confirmed_checksums: Optional[Set[str]] = None
    ...
    def on_file_changed(self, path):
        if self._data_available:
            # увеличенный debounce: при массовой перезаписи 300 файлов
            # не запускаем reload на каждый файл
            self._reload_timer.start(1200)
    ...
    def start_async_reload(self):
        ...
        self.loader_thread = NewsLoaderThread(
            self.storage.data_dir,
            self.settings,
            self.storage.last_read_by_channel,
            self.storage._channel_cache
        )
    ...
    def handle_loaded_data(self, result):
        ...
        channels_cnt = defaultdict(int)
        for n in notifiable_new_news:
            channels_cnt[n.channel] += 1

        if len(channels_cnt) == 1:
            ch, count = next(iter(channels_cnt.items()))
            if count == 1:
            ...
            sorted_channels = sorted(channels_cnt.items(), key=lambda x: -x[1])
            all_sound_enabled = all(self.settings.is_sound(ch) for ch in channels_cnt.keys())
    ...
    def show_toast(self, channel: str, message: str, title: str = "",
                   sound_enabled_for_channel: Optional[bool] = None):
